...
public class ExcelMaxTimeDataHandler extends AbstractDataHandler {
    ...
    public void start() {
        ...
        for (AdditionalInfo addInfo : additionalInfos) {
            ...
            row.createCell(0).setCellFormula("MAX(Sheet0!A" + addInfo.rowStart + ":A" + addInfo.rowEnd + ")");
            for (int i = 1; i < methodsCount; i++) {
                char col = (char) ('A' + (char) i * 2 - 1);
                row.createCell(i).setCellFormula("MAX(Sheet0!" + col + addInfo.rowStart + ":" + col + addInfo.rowEnd + ")");
            }
            rowIndex++;
        }
    }
